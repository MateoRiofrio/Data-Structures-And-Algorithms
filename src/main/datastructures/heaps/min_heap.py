class MinHeap:
    ...
    def __siftup(self):
        index = self.length - 1
        while(self.__greater(self.__parent(index), index) and index > 0):
    ...
    def __siftdown(self):
        ...
        while(self.__left(index) < self.length):
            ...
            # if node's right child is less than the left child
            if (self.__right(index) < self.length and self.__less(self.__right(index), smaller_child)):
                smaller_child += 1 # this is now the right child

            # break if node is in correct position (smaller than its children)
            if (self.__less(index, smaller_child)):
                break
